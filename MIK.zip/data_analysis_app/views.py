...
def data_tab(request):
    ...
    df = pd.read_csv("C:\\Users\\Justin\\OneDrive\\Desktop\\TEAM 1.csv")
...
def box_plot(request):
    # Read TEAM1 dataset from CSV
    df = pd.read_csv("C:\Users\Justin\OneDrive\Desktop\TEAM 1.csv")

    # Default settings for the plot
    default_category = 'countries'
    default_value = 'Export_auto'
...
def exploratory_data_analysis_tab(request):
    ...
    default_x_feature = 'EXPORT'
    default_y_feature = 'IMPORT'
    ...
    fig_scatter = px.scatter(df, x=selected_x_feature, y=selected_y_feature, color='countries',
...
def export_to_csv(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="TEAM1.csv"'
